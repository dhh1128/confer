...
**Version:** 1.0 — April 2026 (Python-flavored copy, adapted from origin-platform 2026-05-27)
**Status:** Living document; update alongside `this.i` when practice evolves.

**Project-specific adaptations** from the canonical methodology at
`~/code/origin-platform/docs/methodology.md`: §3 trigger list uses Python type vocabulary
(class, dataclass, enum, Protocol, TypedDict) instead of Java; §8 substitutes GitHub issues
for Jira and uses `#` comment syntax; §9 substitutes `uv run pytest` for `mvn test`. The
substantive rules (rebuttal-surface standard, separate-and-prior `this.i` commits, gate
ceremony, adversarial review) are unchanged.
...
- Any new public type: class, dataclass, enum, `Protocol`, `TypedDict`, or module-level `Callable` signature
- Any new class or module-level function that embodies a behavioral invariant (even if `_private`)
...
- Any deviation from a project standard — coverage, dependency rules, Python version, test
...
**A name that requires a comment to understand is a design smell.** If the docstring sentence
...
```python
# TECH_DEBT: <name, e.g., "Refactor X to Y">
# Optional explanation — especially what future feature or maturity milestone depends on resolution.
```

This project does not use a ticket tracker. For debt that crosses module boundaries, affects
performance or security, or blocks future work, open a GitHub issue in the `dhh1128/confer`
repository with the `tech-debt` label and reference it in the comment (e.g.,
`# TECH_DEBT: Refactor X to Y (gh#42)`). For small local cleanups, the comment alone is enough.
When debt is paid off, remove the comment and close the issue.

Undocumented debt is more dangerous than documented debt: the next developer will fix it
incorrectly, not knowing it was intentional.
