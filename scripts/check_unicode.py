...
confer note: this.i and docs deliberately use em-dashes (U+2014), arrows,
and box-drawing rules (U+2500-block) — none of which are in the dangerous
set below, so the gate stays green on honest content. See this.i uc7nqx4p.
"""
...
# Tune for this repo: exclude generated/minified files. confer has no test
# fixtures that intentionally embed control characters (the gate's own test
# builds them with chr() escapes), so nothing else needs excluding.
