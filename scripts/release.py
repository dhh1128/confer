...
def readme_url():
    """Return the GitHub README URL, or None.

    Prefers the Repository URL baked into installed package metadata so the
    script works correctly when installed anywhere on PATH.  Falls back to
    deriving the URL from the git remote, which works when running directly
    from a source checkout that has not been installed as a package.
    """
    # 1. Installed package metadata (reliable after pip install / bundle install).
    try:
        from importlib.metadata import PackageNotFoundError, metadata
        meta = metadata("confer")
        for entry in meta.get_all("Project-URL") or []:
            label, _, url = entry.partition(", ")
            if label.strip().lower() == "repository":
                return f"{url.strip()}/blob/main/README.md#releasing"
    except PackageNotFoundError:
        pass

    # 2. Git remote (fallback for uninstalled source checkouts).
    try:
...
def run_tests():
    print("Running tests...")
    # confer's project standard (AGENTS.md): uv run pytest, which enforces
    # --cov-branch --cov-fail-under=100 — so the release self-gates on coverage.
    run(["uv", "run", "pytest"])
